...
const emit = defineEmits(["close-modal", "image-generated"]);
...
const saveImage = async () => {
  try {
    console.log('Guardando imagen generada:', imageUrl.value);
    
    // Guardar la URL directamente en el store
    taskStore.addTask("img_gen", imageUrl.value);
    success("Image saved to tasks");
    emit('image-generated', imageUrl.value); // Emitir el evento con la URL
  } catch (err) {
    console.error('Error al guardar la imagen:', err);
    error("Error saving the image");
  }
};
</script>
