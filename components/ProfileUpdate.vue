  ...
  <div class="w-full max-w-3xl mx-auto p-4">
    ...
    >
      <div class="bg-white p-6 rounded-lg shadow-xl flex items-center">
        <svg
          class="animate-spin h-6 w-6 text-blue-500 mr-3"
    ...
    <div
      class="border border-[#e0e0ff] rounded-lg shadow-sm"
      v-if="!initialLoading"
    >
      ...
      <div class="p-6 relative">
        ...
        <div class="grid md:grid-cols-[1fr_auto] gap-6">
          ...
          <div v-if="!isEditingBasic" class="space-y-4">
            <div
              v-if="basicInfoLoadingError"
              class="bg-red-50 p-4 rounded-md text-sm text-red-700"
            >
              <p>
          ...
          <div class="flex justify-center md:justify-end">
            <div
              class="relative w-24 h-24 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden"
            >
              <img
              ...
              />
              <div
                class="absolute inset-0  bg-opacity-0 hover:bg-opacity-50 flex items-center justify-center transition-all"
              >
                <button
        ...
        <div class="space-y-4">
          ...
          <div class="flex justify-between items-center">
            <div class="flex items-center gap-2">
              <div
                class="w-6 h-6 flex items-center justify-center bg-white rounded-full"
              >
            ...
            <div class="flex items-center justify-between flex-1 max-w-[70%]">
              ...
              <button class="text-blue-500 text-sm">Unlink</button>
            </div>
          </div>
          ...
          <div class="flex justify-between items-center">
            ...
            <div class="flex items-center justify-between flex-1 max-w-[70%]">
              <span class="text-sm ml-4 text-gray-500"
                >Account is not linked</span
              >
              <button class="text-blue-500 text-sm">Link</button>
            </div>
          </div>
      ...
      <div class="p-6 border-t border-gray-100 flex items-center gap-2">
        ...
        <div>
          <p class="text-sm">
            You can
            <button class="text-red-500 underline">delete your profile</button>
          </p>
          <p class="text-xs text-gray-500">
            Along with personal data and course progress
          </p>
        </div>
      </div>
    </div>
    ...
    <div
      v-else
      class="border border-[#e0e0ff] rounded-lg shadow-sm p-8 bg-gray-50"
    >
...
import { useUpdateStudentMutation } from "~/composables/useUpdateStudentMutation";
import { useRoute } from "vue-router";

// Obtener ID del estudiante desde los parámetros de la ruta
const route = useRoute();
const studentId =  route.params.studentId?.toString() || ""

// Composición para obtener datos del estudiante
const {
  data: student,
...
// Mutación para actualizar información del estudiante
...
const basicInfoLoadingError = computed(() => !!fetchError.value);
...
const basicInfo = computed(() => {
  ...
  return {
    name: student.value.user_username || "",
...
const loginInfo = computed(() => {
  ...
  return {
    email: student.value.user_email || "",
    phone: student.value.phone || "",
...
// Imagen de perfil
const profileImage = computed(() => student.value?.profile_picture || null);
...
// Modifica la función saveBasicInfo
// Función para actualizar los datos del estudiante de manera reactiva
function updateStudentData(newData) {
  if (!student.value) return;
  
  // Crear un nuevo objeto para asegurar reactividad
  student.value = { ...student.value, ...newData };
}

// Actualizar la función saveBasicInfo
function saveBasicInfo() {
  basicInfoSaving.value = true;

  const formData = new FormData();
  formData.append("name", editBasicInfo.name || "");
  formData.append("surname", editBasicInfo.surname || "");
  if (editBasicInfo.gender) formData.append("gender", editBasicInfo.gender);
  if (editBasicInfo.birthday) formData.append("birthday", editBasicInfo.birthday);
  if (editBasicInfo.phone) formData.append("phone", editBasicInfo.phone);

  updateStudent(
    {
      id: studentId.value || "1",
      data: formData,
    },
    {
      onSuccess: (response) => {
        ...
        // Usar la función optimizada para actualizar datos
        updateStudentData(response.data);
        
        // También forzar una recarga para asegurar sincronización
...
// Actualizar de manera similar saveLoginInfo y handleImageUpload
function saveLoginInfo() {
  loginInfoSaving.value = true;

  const formData = new FormData();
  formData.append("user_email", editLoginInfo.email || "");
  if (editLoginInfo.phone) formData.append("phone", editLoginInfo.phone);
  if (editLoginInfo.password) formData.append("password", editLoginInfo.password);
  ...
  updateStudent(
    ...
    },
    {
      onSuccess: (response) => {
        ...
        showNotification(
          "success",
          response.message || "Información de acceso actualizada exitosamente"
        );
      },
      onError: (error) => {
        showNotification(
          "error",
          error.message || "Error al actualizar la información de acceso"
        );
      },
      onSettled: () => {
  ...
  updateStudent(
    ...
    },
    {
      onSuccess: (response) => {
        ...
        showNotification("success", "Foto de perfil actualizada exitosamente");
      },
      onError: (error) => {
        showNotification("error", "Error al subir la imagen de perfil");
      },
      onSettled: () => {
  ...
  setTimeout(() => {
    try {
      if (Math.random() < 0.1) {
        throw new Error(
          `Could not ${
            socialAccounts.value[platform].connected ? "disconnect" : "connect"
          } ${platform} account`
        );
      }

      socialAccounts.value[platform].connected =
        !socialAccounts.value[platform].connected;
